...
A command-line tool that parses GDShader uniform declarations and generates C# wrapper property code for Godot 4 `[Tool]` scripts.
...
```bash
dotnet run --project tools/uniform_wrapper_generator -- <path-to-gdshader>
```

Example:

```bash
dotnet run --project tools/uniform_wrapper_generator -- shaders/chapter1_foundation/day04_sdf_intro/sdf_intro.gdshader
```

Output to file:

```bash
dotnet run --project tools/uniform_wrapper_generator -- shader.gdshader > output.cs
```
...
- `uniform_wrapper_generator.csproj` — .NET console project
